        ...
        if (ImGui::TreeNode("Alignment"))
        {
            ...
            for (int y = 0; y < 3; y++)
            {
                for (int x = 0; x < 3; x++)
                {
                    ...
                    char name_[32];
                    sprintf(name_, "(%.1f,%.1f)", alignment.x, alignment.y);
                    if (x > 0) ImGui::SameLine();
                    ImGui::PushStyleVar(ImGuiStyleVar_SelectableTextAlign, alignment);
                    ImGui::Selectable(name_, &selected[3 * y + x], ImGuiSelectableFlags_None, ImVec2(80, 80));
                    ImGui::PopStyleVar();
                }
            }
            ImGui::TreePop();
...
static void ShowDemoWindowPopups()
{
    ...
    if (ImGui::TreeNode("Context menus"))
    {
        ...
        static char name_[32] = "Label1";
        char buf[64]; sprintf(buf, "Button: %s###Button", name_); // ### operator override ID ignoring the preceding label
        ImGui::Button(buf);
        if (ImGui::BeginPopupContextItem())
        {
            ImGui::Text("Edit name:");
            ImGui::InputText("##edit", name_, IM_ARRAYSIZE(name_));
        ...
        if (ImGui::BeginTabItem("Colors"))
        {
            ...
            if (ImGui::Button("Export"))
            {
                ...
                for (int i = 0; i < ImGuiCol_COUNT; i++)
                {
                    ...
                    const char* name_ = ImGui::GetStyleColorName(i);
                    if (!output_only_modified || memcmp(&col, &ref->Colors[i], sizeof(ImVec4)) != 0)
                        ImGui::LogText("colors[ImGuiCol_%s]%*s= ImVec4(%.2ff, %.2ff, %.2ff, %.2ff);" IM_NEWLINE, name_, 23 - (int)strlen(name_), "", col.x, col.y, col.z, col.w);
                }
                ImGui::LogFinish();
            }
...
static void ShowExampleMenuFile()
{
    ...
    if (ImGui::BeginMenu("Colors"))
    {
        ...
        for (int i = 0; i < ImGuiCol_COUNT; i++)
        {
            const char* name_ = ImGui::GetStyleColorName((ImGuiCol)i);
            ImVec2 p = ImGui::GetCursorScreenPos();
            ImGui::GetWindowDrawList()->AddRectFilled(p, ImVec2(p.x + sz, p.y + sz), ImGui::GetColorU32((ImGuiCol)i));
            ImGui::Dummy(ImVec2(sz, sz));
            ImGui::SameLine();
            ImGui::MenuItem(name_);
        }
        ImGui::EndMenu();
    }
...
struct MyDocument
{
    ...
    MyDocument(const char* name_, bool open = true, const ImVec4& color = ImVec4(1.0f, 1.0f, 1.0f, 1.0f))
    {
        Name = name_;
        Open = OpenPrev = open;
        Dirty = false;
