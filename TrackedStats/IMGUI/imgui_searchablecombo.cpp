...
bool ImGui::BeginSearchableCombo(const char* label, const char* preview_value, char* input, int input_size, const char* input_preview_value, ImGuiComboFlags flags)
{
    ...
    char name_[16];
    ImFormatString(name_, IM_ARRAYSIZE(name_), "##Combo_%02d", g.BeginPopupStack.Size); // Recycle windows based on depth

    // Peak into expected window size so we can position it
    if (ImGuiWindow* popup_window = FindWindowByName(name_))
    ...
    bool ret = Begin(name_, NULL, window_flags);
    PopStyleVar();
    if (!ret)
    {
        EndPopup();
