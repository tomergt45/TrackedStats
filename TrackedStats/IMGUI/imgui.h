...
namespace ImGui
{
    ...
    IMGUI_API bool          Begin(const char* name_, bool* p_open = NULL, ImGuiWindowFlags flags = 0);
    IMGUI_API void          End();
    ...
    IMGUI_API void          SetWindowPos(const char* name_, const ImVec2& pos, ImGuiCond cond = 0);      // set named window position.
    IMGUI_API void          SetWindowSize(const char* name_, const ImVec2& size, ImGuiCond cond = 0);    // set named window size. set axis to 0.0f to force an auto-fit on this axis.
    IMGUI_API void          SetWindowCollapsed(const char* name_, bool collapsed, ImGuiCond cond = 0);   // set named window collapsed state
    IMGUI_API void          SetWindowFocus(const char* name_);                                           // set named window to be focused / top-most. use NULL to remove focus.
    ...
    IMGUI_API bool          BeginPopupModal(const char* name_, bool* p_open = NULL, ImGuiWindowFlags flags = 0);                     // modal dialog (regular window with title bar, block interactions behind the modal window, can't close the modal window by clicking outside)
